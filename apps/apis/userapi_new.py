...
#     def get(self):
#         captcha, captchaimg = generate_captcha(6)  # 因为我们只需要返回验证码而不再需要图片，所以只需要取出第一个返回值即可
#         captcha_key = f"captcha:{uuid4().hex()}"
#
#         # tips:存入cache,60秒后过期
#         cache.set(captcha_key, captcha)
#
#
#         #将图片转换为base64
#         buf = BytesIO
#         captchaimg.save(buf,format='PNG')
...
#
#         # tips:新，用cache来进行验证处理，从前端获取key
#         captcha_key = cache.get('captcha_key')
#
...
#
#             if imgcode.lower() != stored_captcha.lower():
#                 return {
#                     'msg': '验证码错误'
#                 }, 400
#
